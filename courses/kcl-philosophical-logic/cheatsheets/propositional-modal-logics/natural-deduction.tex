...
\begin{center}
    ...
    &
        ...
        \DisplayProof
    &
        \AxiomC{}\DeduceC{$\lnot\Box!A$}
        \RightLabel{Dual}
        \UnaryInfC{$\Diamond\lnot!A$}
        \DisplayProof
    \\[3em]
...
serial + sym. + trans. & reflexive &  \Ax{D}+\Ax{B}+\Ax{4} & \Ax{T} \\
...
\Log{KTB} (since \Ax{T} gives you \Ax{D}) and why \Log{S5}, i.e.
\Log{KDTB45}, can also be called \Log{KTB4} (since \Ax{B} and
\Ax{4} together give you \Ax{5}) or \Log{KT5} (since \Ax{T} and \Ax{5}
together give you \Ax{B} and \Ax{4}). 
