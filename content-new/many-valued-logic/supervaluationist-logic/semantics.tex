...
A \emph{!!{structure}} for a first-order language $\Lang{L}$ is a triple $\Struct{M}$ consisting of !!a{domain} of objects, $\Domain{M}$, and an interpretation $\Assign{\cdots}{M}$ of !!{constant}s and !!{predicate}s such that:

\begin{itemize}
	\item For a !!{constant} $c$, $\Assign{c}{M}$ is either an object of $\Domain{M}$ or undefined.
	\item For a $n$-place !!{predicate} $R$, $\Assign{c}{M}$ is a function that associate each $n$-tuple to $\True$, $\False$ or $\Undef$.
\end{itemize}
\end{defn}
...
A \emph{supervaluation} $\Struct{M^+}$ of $\Struct{M}$ is !!a{structure} such that $\Domain{M^+}=\Domain{M}$ and $\Assign{\cdot}{M^+}$ is a classical interpretation such that $\Assign{c}{M^+} = \Assign{c}{M^+}$ if the latter is defined, and $\Assign{R}{M^+}$ agrees with $\Assign{R}{M}$ whenever the latter assigns $\True$ or $\False$ (that is, $\Assign{R}{M^+}$ is a set of tuples containing every tuple to which $\Assign{R}{M}$ assigns $\True$ and not containg any tuple to which $\Assign{R}{M}$ assigns $\False$.
\end{defn}

(When applying supervaluationsim to vagueness, we say where $R$ is a $n$-place predicate, $\Assign{R}{M^+}$ assigns $\True$ or $\False$ to every tuple and agrees with $\Assign{R}{M}$ whenever the latter assigns $\True$ or $\False$ to some tuple.)
...
The value of !!a{sentence} $!A$ at a !!a{valuation} $\pAssign{v}$ is $\True$ if $\pValue{v^+}(!A) = \True$ for every supervaluation $\pAssign{v^+}$ for $\pAssign{v}$, $\False$ if $\pValue{v^+}(!A) = \False$ for every supervaluation $\pAssign{v^+}$ for $\pAssign{v}$, and $\Undef$ otherwise.

Analogously, the value of !!a{sentence} $!A$ at !!a{structure} $\Struct{M}$,  is $\Value{!A}{M}$, is $\True$ if $\Value{M^+}(!A) = \True$ for every supervaluation $\Struct{M^+}$ for $\Struct{M}$, $\False$ if $\Value{!A}{M} = \False$ for every supervaluation $\Struct{M^+}$ for $\Struct{M}$, and $\Undef$ otherwise.
\end{defn}

!!^{sentence}s that are true are every supervaluation or false at every supervaluation are sometimes called `supertrue' and `superfalse'. In the traditional supervaluationist picture, these are equated with truth and falsehood. 
