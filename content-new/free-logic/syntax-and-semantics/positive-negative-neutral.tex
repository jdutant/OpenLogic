...
The central point of division is 
...
$$\Atom{\lfrexists}{t} iff \lexists x (\eq[t][x])$$
In neutral (non-bivalent) systems, or in languages without !!{identity}, it
must be introduced as a primitive.
\end{explain}


The rigorous definition of the language of free logic follows the pattern
of that for first-order logic with the simple addition of the existence
predicate. We give it in full here for reference.
...
The language $\Lang {FL}$ of free logic is defined as follows.

\begin{defn}[Terms]
\ollabel{defn:terms}
The set of \emph{terms}~$\Trm[L]$ of~$\Lang {FL}$ is
defined\iftag{fnTerms}{ inductively}{} by:
\begin{enumerate}
\item Every !!{variable} is a term.
\item Every !!{constant} of~$\Lang {FL}$ is a term.
...
The set of \emph{!!{formula}s}~$\Frm[L]$ of the language~$\Lang {FL}$
...
\item If $R$ is an $n$-place !!{predicate} of~$\Lang {FL}$ and $t_1$, \dots,
  $t_n$ are terms of~$\Lang {FL}$, then $\Atom{R}{t_1,\ldots, t_n}$ is an
  atomic !!{formula}.

\item If $t$ is a term of~$\Lang {FL}$, then $\Atom{\lfrexists}{t}$
  is an atomic !!{formula}.

\item If $t_1$ and $t_2$ are terms of~$\Lang {FL}$, then $\Atom{\eq}{t_1, t_2}$
  is an atomic !!{formula}.
