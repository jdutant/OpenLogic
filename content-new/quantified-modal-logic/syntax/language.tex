...
\begin{explain}
Languages of quantified modal logic combines those of
...
\iftag{defNot,defOr,defAnd,defIf,defIff,defTrue,defFalse,defEx,defAll,defBox,defDiamond}{%
...
\begin{explain}
The existence predicate $\lfrexists$ can also be treated as 
defined:

\begin{defn}[Existence predicate]
$\Atom{\lfrexists}{t}$ abbreviates $\lexists v (\eq[t][v])$, 
where $v$ is a variable not free in $t$.
\end{defn}

In all the systems provided in this section, $\Atom{\lfrexists}{t}$
and $\lexists v (\eq[t][v])$ are equivalent. They entail each other 
and each can be proved from the other.
\end{explain}
