...
\begin{figure}
    ...
    {\setlength\extrarowheight{3em}
    \begin{tabular}{ll}
        \AxiomC{}\DeduceC{$\lforall[x][\lnot!A(x)]$}
        \RightLabel{Dual}
        \UnaryInfC{$\lnot\lexists[x][!A(x)]$}
        \DisplayProof
        &
        \AxiomC{}\DeduceC{$\lnot\lexists[x][!A(x)]$}
        \RightLabel{Dual}
        \UnaryInfC{$\lforall[x][\lnot!A(x)]$}
        \DisplayProof
        \\
        \AxiomC{}\DeduceC{$\lexists[x][\lnot!A(x)]$}
        \RightLabel{Dual}
        \UnaryInfC{$\lnot\lforall[x][!A(x)]$}
        \DisplayProof
        &
        \AxiomC{}\DeduceC{$\lnot\lforall[x][!A(x)]$}
        \RightLabel{Dual}
    ...
    \end{defish}
    \caption{Derived Natural Deduction rules for First Order Logic with Identity.}
    \ollabel{natdedfolder}
    
\end{figure}
    
The rules are summarized in \olref{natdedfol} and \olref{natdedfolder}.
...
\subsection{Duality Rules}

\olref{natdedfolder} provides derived Duality rules. Even though you
do not strictly need them, some !!{derivation}s are very long without
them, so I recommend learning them too. 

In exam you may used derived rules unless explicitly instructed 
not to use them.

The Duality rules capture the following equivalences:

    $$\lnot\lforall[x][!A]\liff\lexists[x][\lnot!A]$$

    $$\lnot\lexists[x][!A]\liff\lforall[x][\lnot!A]$$

Note that, unlike MacFarlane's "Quantifier-Negation equivalence" rules,
these rules do not allow you to \emph{substitute} `$\lforall[x]\lnot$' 
by `$\lnot\lexists[x]$' \emph{anywhere in a formula}. They only apply
to the \emph{main} quantifier or negation of a formula. 
