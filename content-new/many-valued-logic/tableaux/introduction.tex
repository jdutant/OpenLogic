...
\subsection{Signed tableaux and closing branches}
...
In bivalent logic, a !!{structure} assigns one, and only one, truth-value
to each !!{formula}. So if a branch requires us to assign both $\True$ and 
$\False$ to !!a{formula}, we close it because we know there is !!{structure} 
no for it.

In many-valued logic, a !!{structure} \emph{also} assigns one,
 and only one, truth-value to each !!{formula}. So a branch
 closes if at assigns \emph{two distinct truth-values} to a given
 !!{formula}, be it $\True$ and $\False$, or $\Undef$ and $\False$,
 or $\True$ and $\Undef$.
