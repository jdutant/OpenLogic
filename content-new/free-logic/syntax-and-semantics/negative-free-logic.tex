...
The guiding idea of negative free logic is that atomic claims are
existence entailing. In English the following inference seems correct:

\begin{enumerate}
  1. René thinks.
  2. Therefore, René exists.
\end{enumerate}

Negative free logic similarly considers that (where $F$ is any predicate)
\Atom{F}{a}$ holds of if $a$ is not empty, hence only if $\lfrexists a$ holds. 
...
In the negative system of free logic, 
...
\tagitem{fnTerms}{\emph{Interpretation of !!{function}s:} for each $n$-place
  !!{function}~$f$ of $\Lang{L}$, an $n$-place partial function $\Assign{f}{M}
