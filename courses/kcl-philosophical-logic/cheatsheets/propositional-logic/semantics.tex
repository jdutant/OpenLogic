...
\begin{defn}
  ...
  \item If $\Gamma$ is a set of !!{formula}s, $\Gamma$ is
    \emph{satisfiable} if there is !!a{valuation}~$\pAssign{v}$ for
    which $\pSat{v}{\Gamma}$, and $\Gamma$ is \emph{unsatisfiable}
    otherwise. (Where, as defined above, $\pSat{v}{\Gamma}$ means that
    $\pSat{v}{\gamma}$ for every $\gamma\in\Gamma$.) 
  \end{enumerate} 
\end{defn}
