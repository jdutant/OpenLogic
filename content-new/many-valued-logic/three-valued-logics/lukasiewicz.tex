...
Another way of thinking of $\Undef$ better fits \L ukasiewicz's choices.
Think of $\True$, $\Undef$ and $\False$ as \emph{degrees of truth}: $\True$ 
is fully true, $\Undef$ is half-true, and $\False$ is not true at all.
Take the conditional to tell us that \emph{the 
 consequent is at least as true as the antecedent}. That is completely 
 true if the consequent is ``more'' true than the antecedent or equally 
 true as the antecdent: hence the $\True$ at all cells on or below the 
 top left - bottom right diagonal of the matrix. That is not true at
 all when the antecedent is fully true and the consequence fully untrue: hence 
  the $\False$ at the top right cell. That is only half true when the 
  consequent is half a truth less than the antecedent: hence the $\Indet$ 
  in the remaining cells. Using $\pValue{v}(!A) - \pValue{v}(!B)$ to 
  represents the `drop' in value from $!A$ to $!B$:
  \begin{itemize}
    \item $\pValue{v}(!A \lif !B) = \True$ if $\pValue{v}(!B) \geq \pValue{v}(!A)$,
    $\pValue{v}(!A) - \pValue{v}(!B)$ otherwise.
  \end{itemize}

That way of thinking of the values can in fact be extended to the other
connectives in \L ukasiewicz's semantics. (This holds also of the Strong
Kleene semantics.) Write $\True - \pValue{v}(!A)$ for the `distance' of 
$!B$'s value to the truth. In \L ukasiewicz's semantics.
\begin{itemize}
  \item $\pValue{v}(!A \land !B) = \mathsf{min}(\pValue{v}(!A),\pValue{v}(!B))$
  \item $\pValue{v}(!A \lor !B) = \mathsf{max}(\pValue{v}(!A),\pValue{v}(!B))$
  \item $\pValue{v}(\lnot !A) = \True - \pValue{v}{!A}$ 
\end{itemize}

Let us now state \L ukasiewicz's semantics formally.
